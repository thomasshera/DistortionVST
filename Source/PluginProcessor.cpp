...
#endif
{
	...
	state->createAndAddParameter("drive", "Drive", "Drive", NormalisableRange<float>(0.0f, 1.0f, 0.0001), 1.0, nullptr, nullptr);
	state->createAndAddParameter("range", "Range", "Range", NormalisableRange<float>(0.0f, 10000.0f, 0.0001), 1.0, nullptr, nullptr);
	state->createAndAddParameter("blend", "Blend", "Blend", NormalisableRange<float>(0.0f, 1.0f, 0.0001), 1.0, nullptr, nullptr);
	state->createAndAddParameter("volume", "Volume", "Volume", NormalisableRange<float>(0.0f, 3.0f, 0.0001), 1.0, nullptr, nullptr);
...
void DistortionVstAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
		...
		for (int sample = 0; sample < buffer.getNumSamples(); sample++) {
			...
			float cleanSig = *channelData; //dry part
			float dirtySig = *channelData; //wet part

			//drive
			dirtySig *= drive * range; //distortion amt * volume multiplier
...
{
    return true; // (change this to false if you choose to not supply an editor)k
}
